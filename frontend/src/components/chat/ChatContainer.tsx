...
export const ChatContainer = ({ userInitials, userName }: ChatContainerProps) => {
    const { messages, isGenerating, error, sendMessage, stopGeneration } = useChatStream();
        ...
        <div className="flex flex-col h-screen max-h-screen">
            ...
            <header className="fixed top-0 inset-x-0 h-16 bg-background/80 backdrop-blur-md border-b border-border/50 flex items-center px-4 md:px-6 z-50 shadow-sm">
                <div className="flex items-center space-x-2">
                    ...
                    <div>
                        <h1 className="text-lg font-semibold tracking-tight leading-none text-foreground/90">SyllabiQ</h1>
                        <p className="text-xs text-muted-foreground font-medium mt-0.5">Your intelligent learning assistant</p>
                    </div>
                </div>
            ...
            {/* Chat Area */}
            <ScrollArea className="flex-1 w-full pt-20 pb-4 px-4 overflow-y-auto">
                {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center pt-24 pb-8 max-w-2xl mx-auto">
                        ...
                        <p className="text-muted-foreground text-center max-w-md mb-8 text-[15px] leading-relaxed">
                            I&apos;m SyllabiQ. I can answer questions directly from your university syllabus. What would you like to know today?
                        </p>
                        <span>{isGenerating ? "Yes" : "No"}</span>
            ...
            {/* Input Area */}
            <div className="p-4 md:px-6 md:pb-6 bg-gradient-to-t from-background via-background/95 to-transparent w-full">
                <ChatInput
